...
pub const DEFAULT_CRYPTO_TYPE: &str = "secp256k1";
...
trait CryptoType {
    ...
    fn create_key(&self, seed: Option<&[u8; 32]>) -> IndyResult<(secp256k1::PublicKey, secp256k1::SecretKey)>;
    fn validate_key(&self, _vk: &secp256k1::PublicKey) -> IndyResult<()>;
    fn sign(&self, sk: &secp256k1::SecretKey, doc: &[u8]) -> IndyResult<secp256k1::Signature>;
    fn verify(&self, vk: &secp256k1::PublicKey, doc: &[u8], signature: &secp256k1::Signature) -> IndyResult<bool>;
...
impl CryptoService {
    ...
    pub fn create_key(&self, key_info: &KeyInfo) -> IndyResult<Key> {
        ...
        let mut vk = vk.serialize().to_base58();
        let sk = sk[..].to_base58();
    ...
    pub fn create_my_did(&self, my_did_info: &MyDidInfo) -> IndyResult<(Did, Key)> {
        ...
        let did = match my_did_info.did {
            ...
            _ if my_did_info.cid == Some(true) =>
                DidValue::new(&vk.serialize().to_vec().to_base58(), my_did_info.method_name.as_ref().map(|method| method.0.as_str())),
            _ =>
                DidValue::new(&vk.serialize()[0..16].to_vec().to_base58(), my_did_info.method_name.as_ref().map(|method| method.0.as_str()))
        };
    ...
    pub fn sign(&self, my_key: &Key, doc: &[u8]) -> IndyResult<Vec<u8>> {
        ...
        let my_sk = secp256k1::SecretKey::from_slice(&my_key.signkey.as_str().from_base58()?.as_slice());
        let signature = crypto_type.sign(&my_sk.unwrap(), doc);

        trace!("sign <<< signature: {:?}", signature);

        Ok(signature.unwrap().serialize_compact().to_vec())
    }
    ...
    pub fn verify(&self, their_vk: &str, msg: &[u8], signature: &[u8]) -> IndyResult<bool> {
        ...
        let their_vk = secp256k1::PublicKey::from_slice(&their_vk.from_base58()?).unwrap();
    ...
    pub fn convert_seed(&self, seed: Option<&str>) -> IndyResult<Option<[u8; 32]>> {
        ...
        let bytes:[u8; 32]= if seed.as_bytes().len() == 32 {
            // is acceptable seed length
            array_ref!(seed.as_bytes(), 0, 32).clone()
            ...
            */
            ...
            //}
        } else if seed.as_bytes().len() == 64  {
            // is hex string
            let decode_hex = Vec::from_hex(seed).unwrap();
            array_ref!(&decode_hex[..], 0, 32).clone()
        } else {
            return Err(err_msg(IndyErrorKind::InvalidStructure,
